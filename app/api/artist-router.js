...
export default class ArtistRouter extends BaseRouter {
  ...
  setupRoutes() {
    ...
    this.app.get("/api/artists/:name", (req, res) =>
      this.getArtistByName(req, res)
    );
    this.app.post("/api/artists", this.authorizeMod.bind(this), (req, res) =>
      this.addArtist(req, res)
    );
    this.app.post(
      "/api/artists/:id",
      this.authorizeMod.bind(this),
      (req, res) => this.updateArtist(req, res)
    );
    this.app.get("/api/similar-artists", (req, res) =>
      this.getSimilarArtists(req, res)
    );
    this.app.get("/api/similar-banned-artists", (req, res) =>
      this.getSimilarBannedArtists(req, res)
    );
  }
  ...
  async getAllArtists(req, res) {
    try {
      let query =
        "SELECT Id AS id, Name AS name, PatreonName AS patreonName, E621Name AS e621Name FROM artist";
  ...
  async getArtistByName(req, res) {
    try {
      ...
      let artistDataQuery =
        "SELECT Id, E621Name, PatreonName from artist where Name = ?";
      let linksQuery =
        "SELECT LinkType as linkType, LinkURL as linkUrl FROM artistlink WHERE ArtistId = ?";
      ...
      let [artistE621Name, artistPatreonName] = [
        artistData[0].E621Name,
        artistData[0].PatreonName,
      ];
  ...
  async addArtist(req, res) {
    try {
      let [artistName, e621Name, patreonName] = [
        req.body.artistName,
        req.body.e621Name,
      ...
      if (!artistName || artistName.length < 2) {
        return this.returnApiError(
          res,
          new ApiError("Name must be at least 2 characters long", 400)
        );
      }

      let alreadyExistsQuery = "SELECT * FROM artist WHERE Name = ?";
      let query =
        "INSERT INTO artist (Name, E621Name, PatreonName) VALUES (?, ?, ?)";
      let existingArtist = this.databaseFacade.execute(alreadyExistsQuery, [
        artistName,
      ]);
      ...
      if (err?.error?.code && err.error.code === "ER_DUP_ENTRY") {
        return this.returnApiError(
          res,
          new ApiError("Artist with this name already exists", 400)
        );
      }
      return this.returnApiError(res, err);
    }
  ...
  async updateArtist(req, res) {
    try {
      ...
      let updateQuery =
        "UPDATE artist SET Name=?, E621Name=?, PatreonName=? WHERE Id=?";
      let deleteLinksQuery = "DELETE FROM artistlink WHERE ArtistId=?";
      let insertLinksQuery =
        "INSERT INTO artistlink (ArtistId, LinkURL, LinkType) VALUES ";
      let insertLinksParams = [];

      if (links.length > 0) {
        for (var typedLink of typedLinks) {
          insertLinksQuery += `(?, ?, ?), `;
          insertLinksParams.push(
            artistId,
            typedLink.linkUrl,
            typedLink.linkType
          );
        }
        insertLinksQuery = insertLinksQuery.substring(
          0,
          insertLinksQuery.length - 2
        );
      }
  ...
  async getSimilarBannedArtists(req, res) {
    ...
    try {
      let bannedArtists = await this.databaseFacade.execute(
        bannedArtistsQuery,
        null,
        "Error getting banned artists"
      );
  ...
  async getSimilarArtists(req, res) {
    try {
      ...
      if (!newArtistName || newArtistName.length < 2) {
        return this.returnApiError(
          res,
          new ApiError("ArtistName must be at least 2 characters long", 400)
        );
      }
      newArtistName = newArtistName.toLowerCase();
