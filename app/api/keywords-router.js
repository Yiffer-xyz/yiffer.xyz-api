...
export default class KeywordsRouter extends BaseRouter {
  ...
  async getAllKeywords(req, res) {
    try {
      ...
      let result = await this.databaseFacade.execute(query, null, 'Error getting keywords from database', 'Get all keywords')
      res.json(result)
    }
    catch (err) {
  ...
  async getComicKeywords(req, res) {
    ...
    try {
      let result = await this.databaseFacade.execute(query, [Number(comicId)], 'Error getting comic tags from database', 'Get comic keywords')
      res.json(result)
    }
    catch (err) {
  ...
  async logKeywordClick(req, res) {
    ...
    try {
      await this.databaseFacade.execute(query, queryParams, 'Error logging tag click', 'Log keyword click')
      res.json({ success: true })
    }
    catch (err) {
      return this.returnError(err.message, res, err.error)
