...
export default class MiscRouter extends BaseRouter {
  ...
  setupRoutes() {
    this.app.get(
      "/api/comicsuggestions",
      this.authorizeMod.bind(this),
      (req, res) => this.getComicSuggestions(req, res)
    );
    this.app.get("/api/comicsuggestions/rejected", (req, res) =>
      this.getRejectedComicSuggestions(req, res)
    );
    this.app.post("/api/comicsuggestions", (req, res) =>
      this.addComicSuggestion(req, res)
    );
    this.app.post(
      "/api/comicsuggestions/:id/process",
      this.authorizeMod.bind(this),
      (req, res) => this.processComicSuggestion(req, res)
    );

    this.app.get(
      "/api/comic-problems",
      this.authorizeMod.bind(this),
      (req, res) => this.handleGetComicProblems(req, res)
    );
    this.app.get("/api/comic-problems/:comicId", (req, res) =>
      this.handleGetSingleComicProblemCategories(req, res)
    );
    this.app.post("/api/comic-problems", (req, res) =>
      this.addComicProblem(req, res)
    );
    this.app.delete(
      "/api/comic-problems/:id",
      this.authorizeMod.bind(this),
      (req, res) => this.removeComicProblem(req, res)
    );
    this.app.patch(
      "/api/comic-problems/:id/assign",
      this.authorizeMod.bind(this),
      (req, res) => this.assignComicProblem(req, res)
    );

    this.app.get("/api/comic-problem-categories", (req, res) =>
      this.handleGetProblemCategories(req, res)
    );
    this.app.put("/api/comic-problem-categories/:id", (req, res) =>
      this.updateProblemCategory(req, res)
    );
    this.app.post("/api/comic-problem-categories", (req, res) =>
      this.createProblemCategory(req, res)
    );
    this.app.delete("/api/comic-problem-categories/:id", (req, res) =>
      this.deleteProblemCategory(req, res)
    );

    this.app.get("/api/modlog", this.authorizeAdmin.bind(this), (req, res) =>
      this.getModLog(req, res)
    );
    this.app.get("/api/modscores", this.authorizeMod.bind(this), (req, res) =>
      this.getModScores(req, res)
    );

    this.app.post(
      "/api/swapcomicpages",
      this.authorizeMod.bind(this),
      (req, res) => this.swapComicPages(req, res)
    );
    this.app.post(
      "/api/insertcomicpage",
      this.authorizeMod.bind(this),
      upload.single("newPageFile"),
      (req, res) => this.insertComicPage(req, res)
    );
    this.app.post(
      "/api/deletecomicpage",
      this.authorizeMod.bind(this),
      (req, res) => this.deletecomicpage(req, res)
    );
    ...
    this.app.get(
      "/api/stats/routes",
      this.authorizeMod.bind(this),
      (req, res) => this.getRouteStats(req, res)
    );
    this.app.get(
      "/api/stats/visitors",
      this.authorizeMod.bind(this),
      (req, res) => this.getVisitorStats(req, res)
    );
    this.app.get(
      "/api/stats/comic-views",
      this.authorizeMod.bind(this),
      (req, res) => this.getComicViewStats(req, res)
    );

    this.app.post(
      "/api/mod-applications",
      this.authorizeUser.bind(this),
      (req, res) => this.createModApplication(req, res)
    );
    this.app.get(
      "/api/mod-applications",
      this.authorizeAdmin.bind(this),
      (req, res) => this.getModApplications(req, res)
    );
    this.app.post(
      "/api/mod-applications/:id",
      this.authorizeAdmin.bind(this),
      (req, res) => this.processModApplication(req, res)
    );
    this.app.get(
      "/api/mod-applications/me",
      this.authorizeUser.bind(this),
      (req, res) => this.getMyModApplicationStatus(req, res)
    );

    this.app.post("/api/feedback", (req, res) => this.submitFeedback(req, res));
    this.app.get("/api/feedback", this.authorizeMod.bind(this), (req, res) =>
    ...
    this.app.patch(
      ...
      this.authorizeMod.bind(this),
      (req, res) => this.markFeedbackRead(req, res)
    );

    this.app.get("/api/similar-comics", (req, res) =>
      this.getSimilarComics(req, res)
    );
  }

  async getComicSuggestions(req, res) {
    let query =
      "SELECT comicsuggestion.Id AS id, Name AS name, ArtistName AS artist, Description AS description, user.username AS user, comicsuggestion.UserIP AS userIP FROM comicsuggestion LEFT JOIN user ON (comicsuggestion.User = user.Id) WHERE Processed=0 ORDER BY Timestamp ASC";
    try {
  ...
  async getRejectedComicSuggestions(req, res) {
    let query =
      "SELECT Name AS name, ArtistName AS artist, Reason AS reason FROM comicsuggestion WHERE Approved=0 AND ShowInList=1 ORDER BY Timestamp DESC";
    try {
      let result = await this.databaseFacade.execute(
        query,
        null,
  ...
  async addComicSuggestion(req, res) {
    try {
      let [comicName, artist, comment] = [
        req.body.comicName,
        req.body.artist,
        req.body.comment,
      ];

      let existingSuggestionsQuery =
        "SELECT * FROM comicsuggestion WHERE Name LIKE ?";
      let existingSuggestions = await this.databaseFacade.execute(
        existingSuggestionsQuery,
        [comicName],
        "Database error occurred when fetching existing suggestions list"
      );
      if (existingSuggestions.length > 0) {
        return this.returnApiError(
          res,
          new ApiError("This comic name has already been suggested", 400)
        );
      }

      let existingComicQuery = "SELECT * FROM comic WHERE Name LIKE ?";
      let existingComics = await this.databaseFacade.execute(
        existingComicQuery,
        [comicName],
        "Database error occurred when fetching list of comics"
      );
      if (existingComics.length > 0) {
        return this.returnApiError(
          res,
          new ApiError("A comic with this name already exists!", 400)
        );
      }
      ...
      let query = `INSERT INTO comicsuggestion (Name, ArtistName, Description, ${
        req.userData ? "User" : "UserIP"
      }) VALUES (?, ?, ?, ?)`;
      ...
      await this.databaseFacade.execute(
        query,
        queryParams,
        "Database error occurred when adding suggestion"
      );
      res.status(204).end();
    } catch (err) {
  ...
  async processComicSuggestion(req, res) {
    ...
    try {
      if (isApproved) {
        ...
        await this.processNotApprovedSuggestion(
          res,
          suggestionId,
          shouldShowInList,
          reason
      ...
      let suggestionDetails = (
        await this.databaseFacade.execute(
          "SELECT Name, ArtistName, Description FROM comicsuggestion WHERE Id=?",
          [suggestionId]
        )
      )[0];

      let actionString = isApproved
        ? "Approve"
        : shouldShowInList
        ? "Reject-list"
        : "Reject-spam";
      ...
      this.addModLog(
        req,
        "Comic suggestion",
        `${actionString} ${suggestionDetails.Name}`,
        `${suggestionDetails.Name} by ${suggestionDetails.ArtistName}. User desc: "${suggestionDetails.Description}".${modReasonString}`
      );
    } catch (err) {
      return this.returnError(err.message, res, err.error);
    }
  ...
  async getComicProblems(comicId, problemCategoryId) {
    ...
    let problems = await this.databaseFacade.execute(
      query,
      queryParams,
  ...
  async handleGetProblemCategories(req, res) {
    try {
      let query =
        "SELECT Id AS id, Name AS name, HelperText AS helperText FROM comicproblemcategory ORDER BY Name ASC";
      let categories = await this.databaseFacade.execute(
        query,
        null,
        "Database error getting comic problem categories",
        "Get problem categories"
      );

      let sortedCategories = categories.sort((c1) =>
        c1.name === "Other" ? 0 : -1
      );
  ...
  async updateProblemCategory(req, res) {
    try {
      let [categoryId, name, helperText] = [
        Number(req.params.id),
        req.body.name,
        req.body.helperText,
      ];
      let query =
        "UPDATE comicproblemcategory SET Name = ?, HelperText = ? WHERE Id = ?";
      let queryParams = [name, helperText, categoryId];

      await this.databaseFacade.execute(
        query,
        queryParams,
        "DB error updating category"
      );
  ...
  async createProblemCategory(req, res) {
    try {
      ...
      let query =
        "INSERT INTO comicproblemcategory (Name, HelperText) VALUES (?, ?)";
      let queryParams = [name, helperText];

      let result = await this.databaseFacade.execute(
        query,
        queryParams,
        "DB error creating category"
      );
  ...
  async addComicProblem(req, res) {
    try {
      ...
      if (
        ...
        existingProblem[0].problemCategoryName !== "Other"
      ) {
        return this.returnApiError(
          res,
          new ApiError(
            "This problem has already been reported for this comic",
            409
          )
        );
      }
  ...
  async removeComicProblem(req, res) {
    try {
      ...
      let comicProblem = await this.databaseFacade.execute(
        problemQuery,
        [problemId],
        "Database error getting comic problem"
      );
      ...
      await this.databaseFacade.execute(
        query,
        queryParams,
        "DB error deleting comic problem"
      );
      res.status(204).end();
      ...
      this.addModLog(
        ...
        "Problem",
        `Removed ${
          comicProblem.assignedModId ? "assigned" : "unassigned"
        } problem ${comicProblem.problemCategoryName} from comic ${
          comicProblem.comicName
        }`,
        `Description: ${comicProblem.description}`
      );
    } catch (err) {
      return this.returnApiError(res, err);
    }
  ...
  async assignComicProblem(req, res) {
    try {
      ...
      let existingProblem = await this.databaseFacade.execute(
        existingProblemQuery,
        existingProblemQueryParams,
        "Error getting existing problem"
      );
      if (existingProblem[0].AssignedModId) {
        if (existingProblem[0].AssignedModId === req.userData.id) {
          ...
          return this.returnApiError(
            res,
            new ApiError(
              `Already assigned to mod ${existingProblem[0].AssignedMod}`,
              409
            )
          );
        }
      ...
      let updateQuery =
        "UPDATE comicproblem SET AssignedModId = ? WHERE Id = ?";
      let updateQueryParams = [
        shouldUnassign ? null : req.userData.id,
        problemId,
      ];
  ...
  async processNotApprovedSuggestion(
    res,
    suggestionId,
    shouldShowInList,
    reason
    ...
    let query =
      "UPDATE comicsuggestion SET Processed=1, Approved=0, ShowInList=?, Reason=? WHERE Id=?";
  ...
  async getModLog(req, res) {
    let query =
      "SELECT modlog.Id AS id, user.Username AS username, ActionType AS actionType, ActionDescription AS actionDescription, ActionDetails AS actionDetails, Timestamp AS timestamp FROM modlog INNER JOIN user ON (modlog.User=user.Id) ORDER BY Timestamp DESC";
    try {
      let result = await this.databaseFacade.execute(
        query,
        null,
  ...
  async getModScores(req, res) {
    ...
    try {
      let logs = await this.databaseFacade.execute(
        query,
        queryParams,
        null,
        "Get mod scores"
      ...
      for (var log of logs) {
        ...
        userScores[log.Username] += this.getActionScore(
          log.ActionType,
          log.ActionDescription
        );
      }

      let userScoreList = Object.keys(userScores).map(
        (user) =>
  ...
  getActionScore(actionType, actionDescription) {
    if (actionType === "Comic") {
      ...
      if (
        actionDescription.includes("Approve ") ||
        actionDescription.includes("Reject ")
      ) {
        return 30;
      }
      ...
      if (
        actionDescription.includes(" keywords to ") ||
        actionDescription.includes(" keywords from ")
      ) {
      ...
      console.log(
        "Failed to get action score, type and desc: ",
        actionType,
        actionDescription
      );
      return 0;
  ...
  async swapComicPages(req, res) {
    let [comicName, pageNumber1, pageNumber2] = [
      req.body.comicName,
      req.body.pageNumber1,
      req.body.pageNumber2,
    ];
    ...
    try {
      ...
      this.addModLog(
        req,
        "Comic",
        `Swap pages in ${comicName}`,
        `Page ${pageNumber1} and ${pageNumber2}`
      );
    } catch (err) {
  ...
  async insertComicPage(req, res) {
    ...
    try {
      ...
      let numberOfPagesRes = await this.databaseFacade.execute(
        numberOfPagesQuery,
        [comicId]
      );
      ...
      for (
        let pageNo = numberOfPages;
        pageNo >= insertAfterPageNumber + 1;
        pageNo--
      ) {
        await FileSystemFacade.renameGoogleComicFile(
      ...
      await this.databaseFacade.execute(
        query,
        queryParams,
        "Error updating number of pages"
      );
      ...
      this.addModLog(
        req,
        "Comic",
        `Insert page in ${comicName}`,
        `Page at position ${insertAfterPageNumber + 1}`
      );
    } catch (err) {
      return this.returnApiError(res, err);
    }
  ...
  async deletecomicpage(req, res) {
    ...
    try {
      let numberOfPages = (
        await this.databaseFacade.execute(numberOfPagesQuery, [comicId])
      )[0].NumberOfPages;
      let queryParams = [numberOfPages - 1, comicId];

      await FileSystemFacade.deleteGoogleComicFile(
        `${comicName}/${this.getPageName(pageNumber)}.jpg`
      );
  ...
  async createModApplication(req, res) {
    try {
      let [notes, telegramUsername] = [
        req.body.notes,
        req.body.telegramUsername,
      ];

      let existingApplicationQuery =
        "SELECT * FROM modapplication WHERE UserId = ?";
      let existingApplication = await this.databaseFacade.execute(
        existingApplicationQuery,
        [req.userData.id],
        "Database error: Error listing existing applications"
      );
      if (existingApplication.length > 0) {
        return this.returnApiError(
          res,
          new ApiError("You already have a pending application", 400)
        );
      }

      let addApplicationQuery = `INSERT INTO modapplication (UserId, Notes, CompetentAnswer, TelegramUsername) VALUES (?, ?, ' ', ?)`;
      let addApplicationQueryParams = [
        req.userData.id,
        notes,
        telegramUsername,
      ];
      await this.databaseFacade.execute(
        addApplicationQuery,
        addApplicationQueryParams,
        "Database error: Error adding application"
      );
  ...
  async getModApplications(req, res) {
    let existingApplicationQuery =
      "SELECT modapplication.Id AS id, user.Username AS username, Timestamp AS timestamp, Notes AS notes, TelegramUsername AS telegramUsername, IsProcessed AS isProcessed, isRemoved AS isRemoved FROM modapplication INNER JOIN user ON (user.Id = modapplication.UserId)";

    try {
      let results = await this.databaseFacade.execute(
        existingApplicationQuery,
        null,
        "Error getting mod applications"
      );
      res.json(results);
    } catch (err) {
      return this.returnError(err.message, res, err.error, err);
  ...
  async processModApplication(req, res) {
    ...
    let query =
      "UPDATE modapplication SET IsProcessed=1, IsRemoved=? WHERE Id=?";
  ...
  async getMyModApplicationStatus(req, res) {
    let query =
      "SELECT IsProcessed AS isProcessed, IsRemoved AS isRemoved FROM modapplication WHERE UserId=?";
    try {
      let result = await this.databaseFacade.execute(
        query,
        [req.userData.id],
        "Error getting mod application status"
      );
      if (result.length === 0) {
      ...
      if (result.length !== 1) {
        return this.returnError(
          "Error getting mod application status - not a single application on this user-id",
          res,
          null,
          null
        );
      ...
      if (!application.isProcessed && !application.isRemoved) {
        return res.json({
          applicationStatus: MOD_APPLICATION_STATUSES.pending,
        });
      } else if (application.isProcessed && !application.isRemoved) {
        return res.json({
          applicationStatus: MOD_APPLICATION_STATUSES.waiting,
        });
      } else if (application.isRemoved) {
        return res.json({
          applicationStatus: MOD_APPLICATION_STATUSES.removed,
        });
      }
      return res.json({ applicationStatus: MOD_APPLICATION_STATUSES.none });
    } catch (err) {
  ...
  async getFeedback(req, res) {
    ...
    let query =
      "SELECT feedback.Id AS id, Text AS text, user.Username AS username, IsRead AS isRead, Timestamp AS timestamp FROM feedback LEFT JOIN user ON (user.Id = feedback.UserId) WHERE IsRemoved=0 ORDER BY feedback.Id DESC";

    try {
      let feedbacks = await this.databaseFacade.execute(
        query,
        null,
    ...
    try {
      ...
      let [allComics, pendingComics, uploadComics] = await Promise.all([
        this.databaseFacade.execute(
          allComicsQuery,
          null,
          "Error getting all comics"
        ),
        this.databaseFacade.execute(
          pendingComicsQuery,
          null,
          "Error getting pending comics"
        ),
        this.databaseFacade.execute(
          uploadComicsQuery,
          null,
          "Error getting upload comics"
        ),
      ]);
...
function getNumberOfKeywordsAdded(actionDescription, isPendingComic) {
  ...
  if (isPendingComic) {
    ...
    numberStopIndex = isRemove
      ? actionDescription.indexOf(" from")
      : actionDescription.indexOf(" to");
  }
