...
export default class MiscRouter extends BaseRouter {
  ...
  setupRoutes() {
    this.app.get('/api/comicsuggestions', this.authorizeMod.bind(this), (req, res) =>
      this.getComicSuggestions(req, res)
    );
    ...
    this.app.post('/api/comicsuggestions/:id/process', this.authorizeMod.bind(this), (req, res) =>
      this.processComicSuggestion(req, res)
    );

    this.app.get('/api/comic-problems', this.authorizeMod.bind(this), (req, res) =>
      this.handleGetComicProblems(req, res)
    );
    ...
    this.app.delete('/api/comic-problems/:id', this.authorizeMod.bind(this), (req, res) =>
      this.removeComicProblem(req, res)
    );
    this.app.patch('/api/comic-problems/:id/assign', this.authorizeMod.bind(this), (req, res) =>
      this.assignComicProblem(req, res)
    );
    ...
    this.app.post('/api/insertcomicpage', this.authorizeMod.bind(this), upload.single('newPageFile'), (req, res) =>
      this.insertComicPage(req, res)
    );
    ...
    this.app.get('/api/stats/comic-views', this.authorizeMod.bind(this), (req, res) =>
      this.getComicViewStats(req, res)
    );

    this.app.post('/api/mod-applications', this.authorizeUser.bind(this), (req, res) =>
      this.createModApplication(req, res)
    );
    this.app.get('/api/mod-applications', this.authorizeAdmin.bind(this), (req, res) =>
      this.getModApplications(req, res)
    );
    this.app.post('/api/mod-applications/:id', this.authorizeAdmin.bind(this), (req, res) =>
      this.processModApplication(req, res)
    );
    this.app.get('/api/mod-applications/me', this.authorizeUser.bind(this), (req, res) =>
      this.getMyModApplicationStatus(req, res)
    );
    ...
    this.app.patch('/api/feedback/:id/read', this.authorizeMod.bind(this), (req, res) =>
      this.markFeedbackRead(req, res)
    );

    this.app.get('/api/similar-comics', (req, res) => this.getSimilarComics(req, res));
    this.app.get('/api/all-comics-simple', (_, res) => this.getAllComicsSimple(res));
  }
  ...
  async processComicSuggestion(req, res) {
    ...
    try {
      ...
      let suggestionDetails = (
        await this.databaseFacade.execute('SELECT Name, ArtistName, Description FROM comicsuggestion WHERE Id=?', [
          suggestionId,
        ])
      )[0];
  ...
  async addComicProblem(req, res) {
    try {
      let [comicId, categoryId, description] = [req.body.comicId, req.body.categoryId, req.body.description];
  ...
  async removeComicProblem(req, res) {
    try {
      ...
      this.addModLog(
        req,
        'Problem',
        `Removed ${comicProblem.assignedModId ? 'assigned' : 'unassigned'} problem ${
          comicProblem.problemCategoryName
        } from comic ${comicProblem.comicName}`,
      ...
      if (existingProblem[0].AssignedModId) {
        if (existingProblem[0].AssignedModId === req.userData.id) {
          ...
          return this.returnApiError(
            res,
            new ApiError(`Already assigned to mod ${existingProblem[0].AssignedMod}`, 409)
          );
        }
      }
  ...
  async getModScores(req, res) {
    ...
    try {
      ...
      let userScoreList = Object.keys(userScores).map(
        user => new Object({ username: user, score: Math.round(userScores[user]) })
      );
  ...
  async swapComicPages(req, res) {
    let [comicName, pageNumber1, pageNumber2] = [req.body.comicName, req.body.pageNumber1, req.body.pageNumber2];
    let pageName1 = this.getPageName(pageNumber1);
    let pageName2 = this.getPageName(pageNumber2);

    try {
      await FileSystemFacade.renameGoogleComicFile(`${comicName}/${pageName1}.jpg`, `${comicName}/temp.jpg`);
      await FileSystemFacade.renameGoogleComicFile(`${comicName}/${pageName2}.jpg`, `${comicName}/${pageName1}.jpg`);
      await FileSystemFacade.renameGoogleComicFile(`${comicName}/temp.jpg`, `${comicName}/${pageName2}.jpg`);
  ...
  async getSimilarComics(req, res) {
    try {
      ...
      if (!newComicName || newComicName.length < 2) {
        return this.returnApiError(res, new ApiError('ComicName must be at least 2 characters', 400));
      }
      ...
      let pendingComicsQuery = 'SELECT Name AS name FROM pendingcomic WHERE processed=0';
      ...
      let [allComics, pendingComics, uploadComics] = await Promise.all([
        this.databaseFacade.execute(allComicsQuery, null, 'Error getting all comics'),
        this.databaseFacade.execute(pendingComicsQuery, null, 'Error getting pending comics'),
        this.databaseFacade.execute(uploadComicsQuery, null, 'Error getting upload comics'),
      ]);
  ...
  async getAllComicsSimple(res) {
    try {
      let response = [];

      let allComicsQuery = 'SELECT Name AS name, Id AS comicId FROM comic';
      let pendingComicsQuery = 'SELECT Name AS name, Id AS comicId FROM pendingcomic WHERE processed=0';
      let uploadComicsQuery = `SELECT ComicName AS name, Id AS comicId FROM comicupload WHERE Status = 'pending'`;
