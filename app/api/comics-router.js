...
module.exports = class ComicsRouter extends BaseRouter {
  ...
  setupRoutes () {
	...
	async getComicList (req, res) {
		...
		try {
			let result = this.databaseFacade.execute(query, queryParams)
			result = result.map(comic => {
	...
	async createComic (req, res) {
		...
		try {
			...
			let result = await this.databaseFacade.execute(insertQuery, insertQueryParams, 'Database error creating new comic')
			let comicId = result.insertId
