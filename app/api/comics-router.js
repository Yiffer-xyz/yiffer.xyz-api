...
const testUploadFormat = upload.fields([{ name: 'files' }, { name: 'thumbnail', maxCount: 1 }]);
...
export default class ComicsRouter extends BaseRouter {
  constructor(app, databaseFacade, config, modLogger) {
    ...
    let scheduledPendingCronJob = new CronJob(
      '0 12 * * *',
      this.publishScheduledComics.bind(this),
      null,
      true,
      'Europe/London'
    );
  ...
  setupRoutes() {
    ...
    this.app.post('/api/comics', this.authorizeMod.bind(this), addComicUploadFormat, (req, res) =>
      this.createComic(req, res)
    );
    this.app.post('/api/comics/:id/addpages', this.authorizeMod.bind(this), upload.array('newPages'), (req, res) =>
      this.addPagesToComic(req, res, false, false)
    );
    this.app.post('/api/comics/:id/updatedetails', this.authorizeMod.bind(this), (req, res) =>
      this.updateComicDetails(req, res)
    );
    this.app.post('/api/comics/:id/rate', this.authorizeUser.bind(this), (req, res) => this.rateComic(req, res));
    this.app.post(
      '/api/comics/:id/addthumbnail',
      this.authorizeMod.bind(this),
      upload.single('thumbnailFile'),
      (req, res) => this.addThumbnailToComic(req, res, false)
    );
    this.app.post('/api/comics/:id/auto-resize', this.authorizeMod.bind(this), (req, res) =>
      this.autoResizeComic(req, res)
    );
    ...
    this.app.get('/api/pendingcomics/:name', this.authorizeMod.bind(this), (req, res) =>
      this.getPendingComic(req, res)
    );
    this.app.put('/api/pendingcomics/:id', this.authorizeMod.bind(this), (req, res) =>
      this.updatePendingComic(req, res)
    );
    this.app.post('/api/pendingcomics/:id', this.authorizeAdmin.bind(this), (req, res) =>
      this.processPendingComic(req, res)
    );
    this.app.post('/api/pendingcomics/:id/schedule', this.authorizeMod.bind(this), (req, res) =>
      this.schedulePendingComic(req, res)
    );
    this.app.post(
      '/api/pendingcomics/:id/addthumbnail',
      this.authorizeMod.bind(this),
      upload.single('thumbnailFile'),
      (req, res) => this.addThumbnailToComic(req, res, true)
    );
    this.app.patch('/api/pendingcomics/:id/set-error', this.authorizeAdmin.bind(this), (req, res) =>
      this.setPendingComicError(req, res)
    );
    this.app.post('/api/pendingcomics/:id/addkeywords', this.authorizeMod.bind(this), (req, res) =>
      this.addKeywordsToPendingComic(req, res)
    );
    this.app.post('/api/pendingcomics/:id/removekeywords', this.authorizeMod.bind(this), (req, res) =>
      this.removeKeywordsFromPendingComic(req, res)
    );
    this.app.post(
      '/api/pendingcomics/:id/addpages',
      this.authorizeMod.bind(this),
      upload.array('newPages'),
      (req, res) => this.addPagesToComic(req, res, true, false)
    );
    this.app.post(
      '/api/pendingcomics/:id/replacepages',
      this.authorizeMod.bind(this),
      upload.array('newPages'),
      (req, res) => this.addPagesToComic(req, res, true, true)
    );

    this.app.post('/api/testupload', this.authorizeMod.bind(this), testUploadFormat, (req, res) =>
      this.testPageUoload(req, res)
    );
  }
  ...
  async getComicListPaginated(req, res) {
    try {
      let [categories, tags, keywordIds, search, page, order, shouldGetKeywords] = [
        ...
        req.query.tags,
        req.query.keywordIds,
        req.query.search,
        req.query.page,
        req.query.order,
        req.query.getKeywords,
      ];
      ...
      let [filterQueryString, filterQueryParams, keywordCountString, innerJoinKeywordString] = getFilterQuery(
        categories,
        tags,
        keywordIds,
        search
      ...
      let totalNumberPromise = this.databaseFacade.execute(
        totalPagesQuery,
        filterQueryParams,
        'Error fetching paginated comics',
        'Get comics paginated (get total num)'
      );
  ...
  async getComicsWithKeywords(comics) {
    let getKwQuery =
      'SELECT keyword.KeywordName AS keywordName, keyword.Id AS keywordId, comic.Id AS comicId FROM comic INNER JOIN comickeyword ON (comic.Id = comickeyword.ComicId) INNER JOIN keyword ON (keyword.Id = comickeyword.KeywordId) WHERE comic.Id IN (?) ORDER BY comicId';
    ...
    let keywords = await this.databaseFacade.execute(
      getKwQuery,
      [comicIds],
      'Error fetching tags from database',
      'Get comics (get keywords)'
    );
  ...
  async getFirstPageComics(req, res) {
    let query =
      'SELECT comic.Id AS id, comic.Name AS name, comic.Cat AS cat, comic.Tag AS tag, artist.Name AS artist, comic.Updated AS updated, comic.State AS state, comic.Created AS created, comic.NumberOfPages AS numberOfPages FROM comic INNER JOIN artist ON (artist.Id = comic.Artist) GROUP BY name, id ORDER BY id LIMIT 50';
    try {
      let results = await this.databaseFacade.execute(
        query,
        null,
        'Error getting comics from database',
        'Get comics (first page)'
      );
      res.json(results);
    } catch (err) {
  ...
  async getComicByName(req, res) {
    ...
    try {
      let result = await this.databaseFacade.execute(
        comicDataQuery,
        queryParams,
        'Error getting comic from database',
        'Get single comic by name'
      );
      ...
      let prevLink = await this.databaseFacade.execute(
        prevLinkQuery,
        [comicId],
        'Error getting comic link from database',
        'Get single comic by name (link)'
      );
      if (prevLink.length > 0) {
        comicData.previousComic = prevLink[0].Name;
      }
      let nextLink = await this.databaseFacade.execute(
        nextLinkQuery,
        [comicId],
        'Error getting comic link from database',
        'Get single comic by name (link)'
  ...
  async updatePendingComic(req, res) {
    ...
    let [comicName, artistId, cat, tag, state, nextComic, previousComic] = [
      ...
      req.body.state,
      req.body.nextComic,
      req.body.previousComic,
    ];
    let comicId = Number(req.params.id);

    try {
      let getCurrentNameQuery = 'SELECT Name FROM pendingcomic WHERE Id = ?';
      let nameResult = await this.databaseFacade.execute(
        getCurrentNameQuery,
        [comicId],
        'Error getting pending comic name'
      );
  ...
  async testPageUoload(req, res) {
    try {
      let [files, thumbnail] = [req.files.files, req.files.thumbnail];
      console.log({ files, thumbnail });
      console.log(files.length, thumbnail);
      res.status(200).end('ok! ' + files.length + ' ' + thumbnail.length);
    } catch (err) {
      return this.returnApiError(res, err);
    }
  }
  ...
  async createComic(req, res) {
    try {
      ...
      let [comicName, artistId, cat, tag, state, isMultipart] = [
        ...
        req.body.tag,
        req.body.state,
        req.body.isMultipart,
      ];

      let nextComic = null,
        previousComic = null,
        keywordIds = null;
      if (req.body.nextComic) {
        nextComic = JSON.parse(req.body.nextComic);
      }
      if (req.body.previousComic) {
        previousComic = JSON.parse(req.body.previousComic);
      }
      if (req.body.keywordIds) {
        keywordIds = JSON.parse(req.body.keywordIds);
      }
      ...
      let insertQuery =
        'INSERT INTO pendingcomic (Moderator, Name, Artist, Cat, Tag, NumberOfPages, State, HasThumbnail) VALUES (?, ?, ?, ?, ?, ?, ?, ?)';
      ...
      let insertResult = await this.databaseFacade.execute(
        insertQuery,
        insertQueryParams,
        'Database error creating new comic'
      );
  ...
  async handleMultipartUpload(req, res, newFiles, thumbnailFile) {
    let [multipartNumber, totalNumberOfParts, multipartKey] = [
      Number(req.body.multipartNumber),
      Number(req.body.totalNumberOfParts),
      req.body.multipartKey,
    ];
  ...
  async writeNewComicFiles(filePaths, comicName, originalThumbnailFilePath) {
    ...
    if (originalThumbnailFilePath) {
      await Promise.all([
        ...
        FileSystemFacade.writeGoogleComicFile(
          originalThumbnailFilePath + '-thumbsmall',
          comicName,
  ...
  async addComicLinksToPendingComic(thisComicId, previousComic, nextComic) {
    if (previousComic) {
      ...
      await this.databaseFacade.execute(
        insertQuery,
        insertQueryParams,
        'Error adding prev comic link in new pending comic'
      );
    }
    ...
    if (nextComic) {
      ...
      await this.databaseFacade.execute(
        insertQuery,
        insertQueryParams,
        'Error adding next comic link in new pending comic'
      );
    }
  }
  ...
  async addKeywordsToComic(keywordIds, comicId, isPendingComic = false) {
    ...
    let insertKeywordsQuery = `INSERT INTO ${
      isPendingComic ? 'pendingcomickeyword' : 'comickeyword'
    } (ComicId, KeywordId) VALUES `;
  ...
  async deleteComic(req, res) {
    ...
    try {
      ...
      await this.databaseFacade.txExecute(
        tx,
        deletePendingLinksQuery,
        [comicId, comicId],
        'Could not delete pending comic links'
      );
  ...
  async addPagesToComic(req, res, isPendingComic, deleteExistingPages) {
    ...
    try {
      ...
      if (!isPendingComic) {
        ...
        await this.databaseFacade.execute(updateUpdatedTimeQuery, [comicId], 'Error updating comic updated timestamp');
      }

      let updateNumberOfPagesQuery = `UPDATE ${
        isPendingComic ? 'pendingcomic' : 'comic'
      } SET NumberOfPages = ? WHERE Id = ?`;
  ...
  async updateComicDetails(req, res) {
    let [comicId, oldName, newName, newCat, newTag, newState, newArtistName, previousComic, nextComic] = [
      ...
      req.body.state,
      req.body.artist,
      Number(req.body.previousComic),
      Number(req.body.nextComic),
    ];
    ...
    try {
      ...
      let query =
        'UPDATE comic SET Name = ?, Cat = ?, Tag = ?, State = ?, Artist = (SELECT artist.Id FROM artist WHERE Name = ?) WHERE Id = ?';
  ...
  async renameComicFiles(comicId, oldComicName, newComicName, isPendingComic = false) {
    ...
    allRenamePromises.push(
      FileSystemFacade.renameGoogleComicFile(`${oldComicName}/thumbnail.webp`, `${newComicName}/thumbnail.webp`)
    );
    allRenamePromises.push(
      FileSystemFacade.renameGoogleComicFile(
        `${oldComicName}/thumbnail-small.webp`,
        `${newComicName}/thumbnail-small.webp`
      )
    );
  ...
  async updatePrevAndNextComicLinks(comicId, previousComic, nextComic) {
    if (previousComic) {
      ...
      let updateResults = await this.databaseFacade.execute(
        updateQuery,
        [previousComic, comicId],
        'Error updating comic link'
      );
  ...
  async getPendingComics(req, res) {
    let query =
      "SELECT artist.Name AS artist, pendingcomic.Id AS id, pendingcomic.Name AS name, user.Username AS modName, ErrorText AS errorText, Cat AS cat, Tag AS tag, NumberOfPages AS numberOfPages, State AS state, HasThumbnail AS hasThumbnail, ScheduledPublish AS scheduledPublish, GROUP_CONCAT(DISTINCT KeywordName SEPARATOR ',') AS keywords FROM pendingcomic INNER JOIN artist ON (pendingcomic.Artist=artist.Id) INNER JOIN user ON (user.Id=pendingcomic.Moderator) LEFT JOIN pendingcomickeyword ON (pendingcomickeyword.ComicId = pendingcomic.Id) LEFT JOIN keyword ON (keyword.Id = pendingcomickeyword.KeywordId) WHERE Processed=0 GROUP BY name, numberOfPages, artist, id ORDER BY pendingcomic.Id ASC";
    try {
  ...
  async getPendingComic(req, res) {
    ...
    let keywordsQuery =
      'SELECT KeywordName AS name, keyword.Id AS id FROM pendingcomickeyword INNER JOIN keyword ON (pendingcomickeyword.KeywordId = keyword.Id) WHERE pendingcomickeyword.ComicId = ?';
    ...
    try {
      ...
      if (comicData.length === 0) {
        return this.returnError('No pending comic with that name', res);
      }
      ...
      let artistLinks = await this.databaseFacade.execute(
        linksQuery,
        [comicData.artistId],
        'Error getting artist links'
      );
  ...
  async approvePendingComic(req, res, comicId) {
    let getFullPendingComicDataQuery =
      'SELECT Name, Cat, Tag, NumberOfPages, State, Artist, HasThumbnail FROM pendingcomic WHERE Id = ?';
    ...
    let insertIntoComicQuery =
      'INSERT INTO comic (Name, Cat, Tag, NumberOfPages, State, Artist) VALUES (?, ?, ?, ?, ?, ?)';

    let comicData = await this.databaseFacade.execute(
      getFullPendingComicDataQuery,
      [comicId],
      'Error getting pending comic data'
    );
    comicData = comicData[0];
    if (comicData.hasThumbnail) {
      return this.returnError('Pending comic has no thumbnail', res);
    }

    let keywordIds = await this.databaseFacade.execute(
      getKeywordsQuery,
      [comicId],
      'Error getting pending comic keywords'
    );
    ...
    let updatePendingComicsQueryParams = [
      comicData.Name,
      comicData.Cat,
      comicData.Tag,
      comicData.NumberOfPages,
      comicData.State,
      comicData.Artist,
    ];
    let insertResult = await this.databaseFacade.execute(
      insertIntoComicQuery,
      updatePendingComicsQueryParams,
      'Error adding new comic to database'
    );
    ...
    await this.databaseFacade.execute(
      deletePendingComicQuery,
      [comicId],
      'Error deleting the comic from pending in database'
    );

    if (req) {
      // from approve click, not cron job scheduling
  ...
  async transferLinksFromPendingToLiveComic(pendingComicId, liveComicId) {
    ...
    let comicLinks = await this.databaseFacade.execute(
      getLinksQuery,
      [pendingComicId, pendingComicId],
      'Error getting pending comic links'
    );
  ...
  async setPendingComicThumbnailAndRemoveThumbError(comicId) {
    let getPendingComicDataQuery =
      'SELECT HasThumbnail AS hasThumbnail, ErrorText AS errorText FROM pendingcomic WHERE Id = ?';
  ...
  async addKeywordsToPendingComic(req, res) {
    try {
      ...
      res.json({ success: true });
      let comicName = (await this.databaseFacade.execute('SELECT Name FROM pendingcomic WHERE Id=?', [comicId]))[0]
        .Name;
      this.addModLog(
        req,
        'Pending comic',
        `Add ${keywords.length} keywords to ${comicName}`,
  ...
  async removeKeywordsFromPendingComic(req, res) {
    try {
      ...
      this.addModLog(
        req,
        'Pending comic',
        `Remove ${keywords.length} keywords from ${comicName}`,
        keywords.map(kw => kw.name).join(', ')
      );
    } catch (err) {
      return this.returnApiError(res, err);
